...
protected:
  ...
  void testcompressDecompressWithUncommonParams(
      uint32_t quality, uint32_t window_bits, uint32_t input_block_bits,
    ...
    compressor.init(quality, window_bits, input_block_bits, disable_literal_context_modeling,
                    encoder_mode);
...
TEST_F(BrotliDecompressorImplTest, CompressAndDecompress) {
  ...
  compressor.init(default_quality, default_window_bits, default_input_block_bits, false,
                  Envoy::Compressor::BrotliCompressorImpl::EncoderMode::Default);
...
TEST_F(BrotliDecompressorImplTest, CompressDecompressWithUncommonParams) {
  ...
  for (uint32_t i = 1; i < 8; ++i) {
    testcompressDecompressWithUncommonParams(
        ...
        true,                     // disable_literal_context_modeling
