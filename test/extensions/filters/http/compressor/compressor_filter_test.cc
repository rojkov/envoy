    ...
    setUpFilter(R"EOF(
{
  ...
  void verifyCompressedData() {
    EXPECT_EQ(expected_str_.length(),
              stats_.counter("test.compressor.test.test.total_uncompressed_bytes").value());
    EXPECT_EQ(data_.length(),
              stats_.counter("test.compressor.test.test.total_compressed_bytes").value());
  }
  ...
  void doResponse(Http::TestResponseHeaderMapImpl& headers, bool with_compression,
    ...
    if (with_compression) {
      ...
      EXPECT_EQ(1, stats_.counter("test.compressor.test.test.compressed").value());
    } else {
