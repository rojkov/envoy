...
* The `content_length`, `content_type`, `disable_on_etag_header` and `remove_accept_encoding_header`
  fields in HTTP Gzip filter have been deprecated in favor of `compressor`.
* The statistics counter `header_gzip` in HTTP Gzip filter has been deprecated in favor
  of `header_compressor_used`.
