...
QatzipFilterConfig::QatzipFilterConfig(
    const envoy::config::filter::http::qatzip::v2::Qatzip& qatzip, const std::string& stats_prefix,
    Stats::Scope& scope, Runtime::Loader& runtime)
    : CompressorFilterConfig(
          qatzip.content_length().value(), qatzip.content_type(), qatzip.disable_on_etag_header(),
          qatzip.remove_accept_encoding_header(), stats_prefix + "qatzip.", scope, runtime, "gzip"),
...
unsigned int QatzipFilterConfig::hardwareBufferSizeEnum(
  ...
  case envoy::config::filter::http::qatzip::v2::Qatzip_HardwareBufferSize::
