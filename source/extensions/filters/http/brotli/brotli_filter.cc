...
BrotliFilterConfig::BrotliFilterConfig(
    const envoy::config::filter::http::brotli::v2::Brotli& brotli, const std::string& stats_prefix,
    Stats::Scope& scope, Runtime::Loader& runtime)
    : CompressorFilterConfig(
          brotli.content_length().value(), brotli.content_type(), brotli.disable_on_etag_header(),
          brotli.remove_accept_encoding_header(), stats_prefix + "brotli.", scope, runtime, "br"),
